...
class NewsDigestGenerator:
    ...
    async def _summarize_candidate(self, candidate: NewsCandidate, language_code: str = "ru") -> str:
        ...
        prompt = (
            "Ты пишешь короткую новостную заметку на основе одной статьи или записи из RSS. Не придумывай факты и не "
            "добавляй оценочных суждений, которых нет в источнике.\n"
            "Всегда используй язык канала, который указан в сообщении пользователя; не вставляй слова на других языках, "
            "если язык канала не английский.\n"
            "Формат:\n"
            "- Первая строка: подходящий эмодзи и жирный заголовок, например '⚡️ **Краткое описание события**'.\n"
            "- Затем 2–3 коротких абзаца, которые раскрывают что произошло и ключевые детали.\n"
            "- Последний абзац начинается с жирного 'ПОЧЕМУ ЭТО ВАЖНО:' и объясняет значимость новости.\n"
            "- В самом конце отдельной строкой добавь ссылку на источник в формате [Источник](URL).\n"
            "Разделяй абзацы пустой строкой (двойной перенос строки), пиши лаконично и без рекламного тона."
        )
