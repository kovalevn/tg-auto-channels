...
from sqlalchemy import Boolean, Integer, String, Text, Time, Uuid
...
class Channel(Base):
    ...
    posting_window_start: Mapped[time | None] = mapped_column(Time(timezone=True), nullable=True)
    posting_window_end: Mapped[time | None] = mapped_column(Time(timezone=True), nullable=True)
